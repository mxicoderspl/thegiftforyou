                            ...
                            <table class="table table-primary mb30 table-hover table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <?php if (count($Bankinfo) > 0) { ?>
                                        <?php for ($i = 0; $i < count($Bankinfo); $i++) { ?>   
                                            <tr class="gradeX">
                                                <!--td><?php echo $i + 1; ?></td-->
                                                <td><?php echo ucfirst($Bankinfo[$i]['setting_name']); ?></td>
                                                <td><?php echo ($Bankinfo[$i]['setting_value']); ?></td>
                                                <td class="text-center">
                                                    <a href="#myModal" title="Edit" id="edit_btn" onclick="edit_setting('<?php echo base64_encode($Bankinfo[$i]['setting_id']); ?>');" data-toggle="modal"> <i class="glyphicon glyphicon-edit"></i></a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    <?php } else { ?> 
...
<script type="text/javascript">
    function edit_setting(id)
    {
        ...
        $('#model_data').html('');
        $.ajax({
            url: "<?php echo site_url() . 'Bankinfo/update' ?>",
            type: "POST",
            dataType: "html",
