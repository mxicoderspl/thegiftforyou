...
function insert_img($portid, $varimagename, $ismain) {
        ...
        if ($this->db->insert('portfolio_image', $data)) {
            return $this->db->insert_id();
        } else {
            return false;
        }
...
function delete_img_byid($portid, $intid) {
        if ($this->db->delete('portfolio_image', array('id' => $intid, 'portfolio_id' => $portid))) {
            return true;
        } else {
            return false;
        }
...
function get_portimg_byid($imgid) {
        $query = $this->db->get_where('portfolio_image', array('id' => $imgid));
...
function update_imgportid($portid, $imgportid, $filename) {
        $data = array(
            'portfolio_id' => $portid,
            'image' => $filename
          
        );
        $this->db->where('id', $imgportid);

        if ($this->db->update('portfolio_image', $data)) {
            return true;
        } else {
            return false;
        }
...
function get_max_portid() {
 ...
 function setmain_img_byid($portid, $intid) {
        $data = array(
            'is_main' => 1
        );
        $options = array('portfolio_id' => $portid, 'id' => $intid);
        $this->db->where($options);
        if ($this->db->update('image', $data)) {
            $this->db->where(array('portfolio_id' => $portid, 'id !=' => $intid));
            $this->db->update('portfolio_image', array('is_main' => 0));
            return true;
        } else {
            return false;
        }
