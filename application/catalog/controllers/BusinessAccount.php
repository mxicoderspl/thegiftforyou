...
//       $this->form_validation->set_rules('about', 'about', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($check_contry)) {
                $this->session->set_flashdata('error', 'Your given data not valid. Please try again later.');
                redirect('BusinessAccount', 'refresh');
            }
            /*  if(empty($check_contry)){
...
//echo $country;die();
    ...
    public function getstate() {
        ...
        $state = $this->common->select_data_by_id('states', 'name', $code, '*', array());
	...
	$states = $this->common->get_all_record('states', array('id','name'), 'name', 'ASC');
        
	foreach ($states as $st){
        if ($state_id == $st['id']) {
   
            $str .= '<option value="' . $st['id'] . '" selected>' . $st['name'] . '</option>';
        } else {
           
            $str .= '<option value="' . $st['id'] . '">' . $st['name'] . '</option>';
        }
     }
        echo $str;
        die();
